module.exports = [

    {
        "d_Id": "gadgets",
        "products":
            [
                "Mobile Phone", "Computer Table", "Laptop", "Desktop", "Security Camera", "Action Camera", "Gaming Console", "Phone Charger", "Drone", "Earphone", "Headset", "Bluetooth Speaker", "Smart Watch", "Basic Phone", "Nintendo Switch", "Power Bank", "Gameboy"
            ]
    },


    {
        "d_Id": "appliances",
        "products":
            [
                "Vacuum Cleaner", "Washing Machine", "Dryer", "Speaker", "Television", "Refrigerator", "Soundbar", "Air Conditioner", "Humidifier", "Toaster", "Coffee Machine", "Microwave", "Stove", "Wall Fan", "Stand Fan"
            ]
    },


    {
        "d_Id": "homeandliving",
        "products":
            [
                "Mop", "Drill", "Hammer", "Shovel", "Pail", "Ladder", "Lock", "Dining Table", "Cutlery", "Chopsticks", "Bed", "Mattress", "Curtain", "Clock", "Alarm Clock", "Hanger", "Pitcher", "Pillow", "Stool", "Mirror", "Sofa", "Indoor Plant"
            ]
    },


    {
        "d_Id": "schoolsupplies",
        "products":
            [
                "Ballpen", "Eraser", "Crayons", "Watercolor", "Pencil", "Permanent Marker", "Whiteboard Marker", "Highlighter", "Notebook", "Whiteboard", "Sticky Note", "Binder Clip", "Correction Tape", "Correction Fluid", "Glue", "Yarn", "Paper Cutter", "Paper Clip", "Folder"
            ]
    },


    {
        "d_Id": "healthandbeauty",
        "products":
            [
                "Lipstick", "Eyeliner", "Makeup Brush", "Shampoo", "Conditioner", "Soap", "Facial Wash", "Food Supplement", "Medicine", "Moisturizer", "Toothbrush", "Deodorant", "Shaver"
            ]
    },


    {
        "d_Id": "babies",
        "products":
            ["Diaper", "Milk Formula", "Baby Girl Dress", "Baby Boy Dress", "Stroller",
                "Sling Carrier", "Crib", "Baby Bottle"]
    },


    {
        "d_Id": "groceries",
        "products":
            [
                "Coffee", "Tea", "Water", "Chocolate", "Juice", "Whisky", "Tequila", "Vodka", "Corned Beef", "Instant Noodles", "Olive Oil", "Vegetable Oil", "Chip", "Biscuit", "Oatmeal", "Cookie", "Bread", "Cake", "Tomatoes", "Onions", "Garlic", "Ginger", "Beef", "Pork", "Chicken", "Shrimp", "Fish", "Matches", "Dishwashing Liquid", "Bleach", "Toilet Cleaner", "Soy Sauce", "Vinegar", "Fish Sauce", "Ketchup", "Mustard", "Eggs", "Mayonaise"
            ]
    },


    {
        "d_Id": "pets",
        "products":
            [
                "Leash", "Collar", "Pet Shoes", "Pet Bowl", "Bird Feeder", "Pet Cage", "Pet Crate", "Pet Litter", "Pet Carrier", "Aquarium", "Pet Toy", "Dog Food", "Cat Food", "Fish Food", "Bird Food"
            ]
    },


    {
        "d_Id": "fashionwomen",
        "products":
            [
                "Tote Bag", "Travel Bag", "Dress", "Coin Purse", "Women Jacket", "Women Sweatshirt", "Women Jeans", "Women Leggings", "Women Skirt", "Women Shorts", "Women Sandals", "Flat Shoes", "Heels", "Wedges", "Boots", "Women Wallet", "Belt Bag", "Women Backpack", "Women Shirt"
            ]
    },


    {
        "d_Id": "fashionmen",
        "products":
            [
                "Tshirt", "Polo Shirt", "Men Jacket", "Coat", "Hoodie", "Men Jeans", "Formal Pants", "Men Shorts", "Sneakers", "Loafers", "Black Shoes", "Men Sandals", "Messenger Bag", "Men Backpack", "Men Wallet", "Belt"
            ]
    },


    {
        "d_Id": "accessories",
        "products":
            [
                "Watch", "Necklace", "Earring", "Ring", "Bangle", "Bracelet", "Eyeglasses", "Sunglasses", "Contact Lenses", "Headband", "Cap"
            ]
    },


    {
        "d_Id": "sportsandlifestyle",
        "products":
            [
                "Bike", "Fishing Rod", "Fishing Hook", "Tent", "Swiss Knife", "Men Sport Shoes", "Women Sport Shoes", "Scooter", "Skateboard", "Yoga Mat", "Treadmill", "Weights", "Basketball", "Boxing Gloves", "Boxing Bag", "Snorkling Gear", "Badminton", "Shuttlecock", "Tennis", "Tennis Ball", "Volleyball", "Baseball", "Baseball Bat", "Baseball Gloves", "Guitar", "Piano", "Electric Keyboard", "Drum Set", "Tambourine", "Snare Drum", "Cymbal", "Xylophone", "Flute", "Recorder", "E-Cigarette", "Guitar Capo", "Harmonica", "Kalimba", "Ukelele"
            ]
    },


    {
        "d_Id": "toysandcollectibles",
        "products":
            [
                "Pokemon Cards", "YuGiOh Cards", "Uno", "Monopoly", "Snake and Ladder", "The Generals", "Lego", "Stuffed Toy", "Key Chain", "Chess", "Crocodile Dentist", "Rubiks Cube", "Jenga", "Spiderman Costume", "Elsa Costume", "Akatsuki Costume", "Beyblade", "Hot Wheels", "Spirograph", "Nerf Gun", "Clay", "Fidget Spinner", "Cards Against Humanity"
            ]
    },


    {
        "d_Id": "automotive",
        "products":
            [
                "Motorcycle", "Helmet", "Anti-freeze", "Brake Fluid", "Grease", "Power Steering Fluid", "Transmission Fluid", "Sealer", "Engine Oil"
            ]
    }

]